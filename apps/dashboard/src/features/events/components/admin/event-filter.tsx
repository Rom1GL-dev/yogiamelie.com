...
interface EventFilterProps {
    selectedType: string;
    eventTypes: { key: string; label: string; count: number }[];
    onSelect: (type: string) => void;
}

export const EventFilter = ({ selectedType, eventTypes, onSelect }: EventFilterProps) => {
    const [dropdownOpen, setDropdownOpen] = useState(false);

    return (
        <div className="flex flex-col lg:flex-row gap-x-4 w-full">
            <div className="hidden lg:flex flex-wrap gap-x-4 w-full">
                {eventTypes.map(({ key, label, count }) => (
                    <div
                        key={key}
                        ...
                        onClick={() => {
                            localStorage.setItem("eventType", key);
                            onSelect(key);
                        }}
                    >
                        {label}
            ...
            <div className="lg:hidden w-3/5 sm:w-2/5 md:w-1/5">
                <div className="relative">
                    ...
                    >
                        {eventTypes.find((type) => type.key === selectedType)?.label || "Filtrer par"}
